...
class FakeTransport(Transport, transport_type="fake-test"):
    ...
    RESOURCE_EXHAUSTED when ``program_write_refused`` is set.
    """
...
def test_unanswered_request_fails_the_connection():
    """The transceiver answers most requests promptly, so an unanswered one
...
def test_avss_program_write_refused_by_the_transceiver():
    """A write the transceiver's flow control refused was never sent, so the
